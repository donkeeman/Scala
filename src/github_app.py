...
async def handle_pr_review(payload: dict):
    ...
    try:
        ...
        diff_text = format_diff_for_llm(file_diffs)
        print(f"[Review] Got diff ({len(file_diffs)} files, {len(diff_text)} chars)")

        review_result = review_diff(diff_text)
        print(f"[Review] Generated review: {len(review_result['comments'])} inline comments")
